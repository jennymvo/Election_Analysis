...
with open(file_to_save, "w") as txt_file:
    ...
    for countyName in countyList:
        ...
        voteCount = countyVotes[countyName]

        # 6c: Calculate the percentage of votes for the county.
        votePercentage = float(voteCount) / float(total_votes) * 100


         # 6d: Print the county results to the terminal.
        countyResults = (
            f"{countyName}: {votePercentage:.1f}% ({voteCount:,})\n")
        ...
        if (voteCount > countyVoteCount) and (votePercentage > winningCountyPercentage):
            winningCounty = countyVoteCount
            winningCountyPercentage = votePercentage
